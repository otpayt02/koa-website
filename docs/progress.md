...
- Verified after rebasing onto remote `ce74fd1`: desktop film resolves to 18 viewport heights (`1800vh`); cursor sample increases from alpha sum `13` to `621`; header occlusion remains `0`; glimmer opacity reaches `0.706`; desktop/mobile overflow is `0`; Motion off settles canvas opacity to `0`; browser error and warning lists are empty.
