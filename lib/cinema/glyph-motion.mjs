...
export function advanceParticle(particle, { deltaMs, elapsedMs = 0 }) {
  ...
  const modeSpeed = particle.mode === "forming" ? 1.15 : particle.mode === "dispersing" ? 0.92 : 0.48;
  const maximumSpeed = speedForAnchorDistance(anchorDistance, modeSpeed);

  if (targetDistance > 0.0001) {
    const acceleration = Math.min(0.065, targetDistance / 1800) * frameScale;
