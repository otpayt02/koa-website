...
export function DonationForm({ lang }: { lang: Lang }) {
  ...
  return (
    <form className="donation-card" action="/api/donations" method="post">
      <p className="eyebrow">{lang === "karen" ? "မၤစၢၤတၢ်မၤ" : "Sustain the work"}</p>
      <h2>{lang === "karen" ? "နတၢ်ဟ့ၣ်မၤစၢၤပှၤတဝၢ။" : "Your gift stays with community."}</h2>
      ...
      <button className="button button--primary" type="submit">{lang === "karen" ? "ဟ့ၣ်မၤစၢၤ" : `Donate $${amount}${recurring ? "/month" : ""}`}</button>
