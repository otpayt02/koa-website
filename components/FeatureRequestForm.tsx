...
export function FeatureRequestForm({ lang, messages }: { lang: Lang; messages: Messages }) {
  return (
    <AsyncForm endpoint="/api/collaborate" messages={messages}>
      <div className="form-intro"><p className="eyebrow">{lang === "karen" ? "တဲဖျါနတၢ်ထံၣ်" : "Shape what comes next"}</p><h2>{lang === "karen" ? "ပအဲၣ်ဒိးနၢ်ဟူနတၢ်ထံၣ်။" : "Tell us what community needs."}</h2></div>
