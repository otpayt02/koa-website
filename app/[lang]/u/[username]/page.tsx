...
import { content, dictionaryEntries, interpreters } from "@/components/data";
...
export default async function ProfilePage({ params }: { params: Promise<{ lang: string; username: string }> }) {
  ...
  return (
    <>
      ...
      <Section tone="cream" eyebrow="Contributions" title="Language work connected to this profile." intro="Contributions are shown only after community review and remain linked to their version history.">{contributions.length ? <div className="dictionary-grid">{contributions.map((entry) => <article className="dictionary-card" key={entry.id}><div className="dictionary-card__top"><StatusPill tone="green">{messages.communityReviewed}</StatusPill><span>v{entry.version}</span></div><p className="dictionary-word" lang="ksw">{entry.word}</p><p className="romanization">{entry.romanization}</p><p>{content(lang, entry.definition.en, entry.definition.karen)}</p><Link className="text-link" href={`/${lang}/dictionary/${entry.id}`}>View dictionary entry</Link></article>)}</div> : <div className="empty-state"><h3>No dictionary contributions published yet.</h3><p>Profile activity will appear here as approved work is connected.</p></div>}</Section>
