...
export default async function CommunityPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return <>
    ...
    <Section tone="cream" eyebrow="Community board" title={t("community.board.title")} intro={t("community.board.intro")}>
      <div className="discussion-list">{discussions.slice(0, 3).map((item) => <article key={item.title}><div><StatusPill tone={item.status === "Approved" ? "green" : "gold"}>{item.status}</StatusPill><h3>{item.title}</h3><p>{item.kind} · {item.author} · {item.time}</p></div><strong>{item.replies}<small>replies</small></strong></article>)}</div><div className="section-action"><Button href={`/${lang}/community/board`}>Open the community board</Button><Button href={`/${lang}/verifier`} variant="quiet">Become a verifier</Button></div>
    </Section>
  </>;
}
